...
public class ZCurseForgeIntegration extends CurseForgeIntegration {
    ...
    private NexusLogger logger = NexusUtilities.getLogger();

    public ZCurseForgeIntegration(NexusLogger logger, int id, int v, String path) throws Exception {
        super(new Logger("CURSE",null), Path.of(path+"/cache/"));
        this.id = id;
        this.v = v;
        pathString = path;
                    ...
                    if(cache.isDirectory()) {
                        ...
                        for(File element:elements) {
                            if(element.getName().toLowerCase().endsWith(".zip")) {
                                ...
                                NexusUtilities.getLogger().deb(element.getName()+" to "+ "temp/"+tempID+"/");
                                new File("temp/"+tempID+"/").mkdirs();
                                if(FileExtractor.unzipFile(element.getAbsolutePath(), "temp/"+tempID+"/")) {
                                    if(new File("temp/"+tempID+"/overrides/").exists()) {
                                        if(new File("temp/"+tempID+"/overrides/").isDirectory()) {
                                            File zip = new File("temp/"+tempID+"/overrides/");
