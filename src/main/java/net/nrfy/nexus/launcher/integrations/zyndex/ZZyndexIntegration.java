package net.nrfy.nexus.launcher.integrations.zyndex;
...
public class ZZyndexIntegration {
    ...
    public static boolean install(ReadableZynstance onlineInstance, String path) {
        try {
            ...
            NexusUtilities.getLogger().deb("[CONNECTOR] Created instance path: " + instance.mkdirs());
            FileGetter.downloadFile(url, URLDecoder.decode(instance.getAbsolutePath() + "/zyneonInstance.json", StandardCharsets.UTF_8));
            return true;
        } catch (Exception e) {
            NexusUtilities.getLogger().err("[ZYNDEX] Couldn't install instance: " + e.getMessage());
            return false;
        }
    }

    public static boolean install(ReadableZynstance onlineInstance, String id, String path) {
        try {
            String url = onlineInstance.getLocation();
        ...
        try {
            ...
            if (fixConverted || !localInstance.getVersion().equals(onlineInstance.getVersion()) || !new File(localInstance.getPath() + "meta/pack.zip").exists()) {
                if (onlineInstance.getDownloadUrl() != null) {
                    NexusUtilities.getLogger().log("[ZYNDEX] Trying to update" + onlineInstance.getName() + " (" + onlineInstance.getId() + ")...");
                    try {
                        String path = localInstance.getPath();
                        File pack = new File(path + "meta/pack.zip");
                        File mods = new File(path + "mods/");
                        File libraries = new File(path + "libraries/");
                        NexusUtilities.getLogger().log("[ZYNDEX] Deleting old libraries...");
                        String lT = "!";
                        try {
                        ...
                        if(!libraries.exists()) {
                            NexusUtilities.getLogger().log("[ZYNDEX] Old libraries deleted!");
                        } else {
                            NexusUtilities.getLogger().err("[ZYNDEX] Couldn't delete old libraries"+lT);
                        }
                        NexusUtilities.getLogger().log("[ZYNDEX] Checking if old pack file exists...");
                        if (pack.exists()) {
                            NexusUtilities.getLogger().log("[ZYNDEX] Found old pack file!");
                            NexusUtilities.getLogger().log("[ZYNDEX] Deleting old pack file...");
                            if (pack.delete()) {
                                NexusUtilities.getLogger().log("[ZYNDEX] Deleted old pack file!");
                            } else {
                                NexusUtilities.getLogger().deb("[ZYNDEX] Failed to delete old pack file. Trying again...");
                                if (pack.delete()) {
                                    NexusUtilities.getLogger().log("[ZYNDEX] Deleted old pack file!");
                                } else {
                                    NexusUtilities.getLogger().err("[ZYNDEX] Couldn't delete old pack file...");
                                    throw new RuntimeException("Couldn't delete file...");
                                }
                            }
                        } else {
                            NexusUtilities.getLogger().log("[ZYNDEX] No old pack file found!");
                        }
                        ...
                        NexusUtilities.getLogger().log("[ZYNDEX] New pack file downloaded!");
                        NexusUtilities.getLogger().log("[ZYNDEX] Deleting old mods...");
                        String mT = "!";
                        try {
                        ...
                        if(!mods.exists()) {
                            NexusUtilities.getLogger().log("[ZYNDEX] Old mods deleted!");
                        } else {
                            NexusUtilities.getLogger().err("[ZYNDEX] Couldn't delete old mods"+lT);
                        }
                        NexusUtilities.getLogger().log("[ZYNDEX] Old mods deleted!");
                        NexusUtilities.getLogger().log("[ZYNDEX] Unzipping pack file...");
                        if (FileExtractor.unzipFile(pack.getPath(), path)) {
                            NexusUtilities.getLogger().log("[ZYNDEX] Pack file unzipped!");
                        } else {
                            NexusUtilities.getLogger().err("[ZYNDEX] Failed to unzip pack file!");
                        ...
                        if(onlineInstance.getBackgroundUrl()!=null||onlineInstance.getIconUrl()!=null||onlineInstance.getLogoUrl()!=null||onlineInstance.getThumbnailUrl()!=null) {
                            ...
                            if(onlineInstance.getIconUrl()!=null) {
                                NexusUtilities.getLogger().log("[ZYNDEX] Downloading icon...!");
                            ...
                            if(onlineInstance.getLogoUrl()!=null) {
                                NexusUtilities.getLogger().log("[ZYNDEX] Downloading logo...!");
                            ...
                            if(onlineInstance.getThumbnailUrl()!=null) {
                                NexusUtilities.getLogger().log("[ZYNDEX] Downloading thumbnail...!");
                        ...
                        NexusUtilities.getLogger().err("[ZYNDEX] Couldn't update. Trying to cancel start...");
                        throw new RuntimeException(e.getMessage());
                    }
                    NexusUtilities.getLogger().deb("[ZYNDEX] Updated instance!");
                }
                localInstance.setMinecraftVersion(onlineInstance.getMinecraftVersion());
                return localInstance;
            }
        } catch (Exception e) {
            NexusUtilities.getLogger().err("[ZYNDEX] Couldn't update " + localInstance.getName() + " (" + localInstance.getId() + "): " + e.getMessage());
        }
        return null;
    }
    ...
    public static File convert(File oldInstanceFile) {
        try {
            ...
            NexusUtilities.getLogger().err("[ZYNDEX] Couldn't convert old instance: " + e.getMessage());
